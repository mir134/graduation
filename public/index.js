...
async function load() {
  ...
  list.innerHTML = data.map((m, index) => {
    ...
    const randomY = Math.floor(Math.random() * 800 - 150);
    return `
					<div id="note-${index}" class="sticky-note absolute p-4 md:p-6 rounded-sm shadow-md" 
						...
						onmouseout="this.style.zIndex=${index}; this.style.transform='rotate(${rotate}deg)'">
							<div class="flex flex-col h-full">
								...
								<p class="font-bold text-gray-900 text-sm md:text-base">💎 ${m.name}</p><p class="text-[14px] text-gray-500 font-mono italic"> 🔐  ${m.contact}</p>
								</div>
							</div>
					</div>`;
  }).join('');
...
async function submit() {
  ...
  btn.disabled = true; btn.innerText = '正在寄出...';
  try {
